...
export const register = (fullname, email, password,phone,address,city,postalCode) => async (dispatch) => {
  dispatch(setLoading(true));
  try {
    const config = {
      headers: {
    ...
    console.log("axios")
    const { data } = await axios.post('/api/users/register', { fullname, email, password,phone,address,city,postalCode }, config);
